...
public class userHandlers implements RouteProvider {
    ...
    public void initRoutes(Environment environment) {
        environment
                .routingEngine()
                .registerAutoRoute(Route.sync("GET", "/", ctx -> Response.ok()
                .withPayload("Successfully reached MATCH - Friend Matching Platform.\n")))
                .registerAutoRoute(Route.sync("GET", "/user/<username>", this::getUser).withMiddleware(jsonMiddleware()))
                .registerAutoRoute(Route.sync("POST", "/register", this::createUser))
                .registerAutoRoute(Route.sync("POST", "/login", this::userLogin))
                .registerAutoRoute(Route.sync("POST", "/user/<username>/logout", this::userLogout));
    }
}
